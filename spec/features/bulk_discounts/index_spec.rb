...
describe 'merchant bulk discount index page' do
  ...
  it 'I see all of my bulk discounts w/percent and quantity threshold' do
    within "#discount-#{@bulk_1.id}" do
      expect(page).to have_content("#{@bulk_1.percent} off of #{@bulk_1.threshold} or more items")
    end
    within "#discount-#{@bulk_2.id}" do
      expect(page).to have_content("#{@bulk_2.percent} off of #{@bulk_2.threshold} or more items")
    end
    within "#discount-#{@bulk_3.id}" do
      expect(page).to have_content("#{@bulk_3.percent} off of #{@bulk_3.threshold} or more items")
    end
    within "#discount-#{@bulk_4.id}" do
      expect(page).to have_content("#{@bulk_4.percent} off of #{@bulk_4.threshold} or more items")
    end
  end
